...
class KalmanFilter:
    ...
    def update(self, y, mu_est, sigma_est, sat_pos):
        ...
        valid_indices1 = np.mod(valid_indices, 11)#find all without repeat
  
        # Initialize 
        self.C = np.zeros((len(valid_indices1), len(mu_est)))
        g = np.zeros(len(valid_indices1))

        for idx, i in enumerate(valid_indices1):
            if (idx > 21):
                n = 2
            elif (idx>10):
                n = 1
